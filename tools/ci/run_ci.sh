...
function run_tests() {
  ...
  # When measuring, LCOV data gets collected always
  lcov --capture --directory . --output-file build-wakaama-tests/lcov.info
  lcov --output build-wakaama-tests/coverage/lcov.info \
    --remove build-wakaama-tests/lcov.info \
    "*/tests/*" \
    "/usr/include/*"
  echo "Coverage file build-wakaama-tests/coverage/lcov.info ready"

  #see https://github.com/koalaman/shellcheck/wiki/SC2089
  gcovr_opts=(-r "${REPO_ROOT_DIR}" --exclude "${REPO_ROOT_DIR}"/build-wakaama-tests --exclude "${REPO_ROOT_DIR}"/tests)
