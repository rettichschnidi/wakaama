...
static int prv_serializeAttributes(lwm2m_context_t *contextP, lwm2m_uri_t *uriP, lwm2m_server_t *serverP,
                                   lwm2m_attributes_t *objectParamP, uint8_t *buffer, size_t uriLen, size_t bufferLen)
{
    int head;
    int res;
    lwm2m_attributes_t *paramP;
...
static int prv_serializeLinkData(lwm2m_context_t *contextP, lwm2m_data_t *tlvP, lwm2m_server_t *serverP,
                                 lwm2m_attributes_t *objectParamP, lwm2m_uri_t *parentUriP, uint8_t *parentUriStr,
                                 size_t parentUriLen, uint8_t *buffer, size_t bufferLen)
{
    int head;
    int res;
    lwm2m_uri_t uri;
    ...
    case LWM2M_TYPE_OBJECT_INSTANCE:
    {
        ...
        {
            res = prv_serializeLinkData(contextP, tlvP->value.asChildren.array + index, serverP, objectParamP, &uri,
                                        uriStr, uriLen, buffer + head, bufferLen - head);
            if (res < 0)
                return -1;
            head += res;
        }
                       ...
                       lwm2m_data_t *dataP, uint8_t **bufferP)
{
    ...
    else
    {
        ...
        if (LWM2M_URI_IS_SET_INSTANCE(uriP))
        {
            ...
            if (serverP != NULL)
            {
                res = prv_serializeAttributes(contextP, &parentUri, serverP, NULL, bufferLink, head - 1,
    ...
    {
        res = prv_serializeLinkData(contextP, dataP + index, serverP, paramP, uriP, baseUriStr, baseUriLen,
                                    bufferLink + head, PRV_LINK_BUFFER_SIZE - head);
        if (res < 0)
            return -1;
        head += res;
    }
